...
app.whenReady().then(async()=>{
 ...
 try{
  ...
  await click('Submit');await contains('Your response');await contains(transcript);if(!await run(`document.querySelector('audio')?.src.startsWith('blob:')`))throw Error('Recording lost on submit');
  const report={date:new Date().toISOString(),ok:true,input:'generated speech through Chromium simulated microphone; no human microphone captured',transcript,checks:['MediaRecorder capture','offline Vosk live transcription','stop and playable blob','submit retains recording and transcript']};fs.writeFileSync(path.join(root,'docs/gpn/recording-report.json'),JSON.stringify(report,null,2)+'\n');console.log('PASS',report);app.exit(0)
